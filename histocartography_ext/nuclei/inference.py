...
def _get_hovernet_class():
    ...
    hovernet_path = Path(__file__).resolve().parents[1] / "ml" / "models" / "hovernet.py"
    if not hovernet_path.is_file():
        raise FileNotFoundError(f"HoverNet implementation not found at: {hovernet_path}")

    spec = importlib.util.spec_from_file_location("_histocartography_ext_hovernet", str(hovernet_path))
    ...
    HoverNet = getattr(module, "HoverNet", None)
    if HoverNet is None:
        raise ImportError(f"No HoverNet class found in: {hovernet_path}")

    _HOVERNET_CLASS = HoverNet
    return HoverNet
...
def _legacy_desc_key_for_state_key(state_key: str) -> Optional[str]:
    ...
    if m:
        pfx = m.group("pfx")
        rest = m.group("rest")
        br = "np" if pfx == "decode_np" else "hv"

        mm = re.match(r"^u(?P<u>[23])_conva\.conv\.weight$", rest)
        if mm:
            u = mm.group("u")
            return f"decoder.{br}.u{u}.conva.weight"

        mm = re.match(r"^u(?P<u>[23])_convf\.conv\.weight$", rest)
        if mm:
            u = mm.group("u")
            return f"decoder.{br}.u{u}.convf.weight"

        if rest == "u1_conva.conv.weight":
            return f"decoder.{br}.u1.conva.weight"

        mm = re.match(r"^u(?P<u>[23])_dense\.blk_bna\.bn\.(?P<suf>.+)$", rest)
        if mm:
            u = mm.group("u")
            suf = mm.group("suf")
            return f"decoder.{br}.u{u}.dense.blk_bna.bn.{suf}"
        ...
        if mm:
            ...
            i = mm.group("i")
            suf = mm.group("suf")
            return f"decoder.{br}.u{u}.dense.units.{i}.preact_bna/bn.{suf}"

        mm = re.match(r"^u(?P<u>[23])_dense\.blk_(?P<i>\d+)conv1\.conv\.weight$", rest)
        if mm:
            u = mm.group("u")
            i = mm.group("i")
            return f"decoder.{br}.u{u}.dense.units.{i}.conv1.weight"
        mm = re.match(
        ...
        if mm:
            ...
            i = mm.group("i")
            suf = mm.group("suf")
            return f"decoder.{br}.u{u}.dense.units.{i}.conv1/bn.{suf}"

        mm = re.match(r"^u(?P<u>[23])_dense\.blk_(?P<i>\d+)conv2\.conv\.weight$", rest)
        if mm:
            u = mm.group("u")
            i = mm.group("i")
            return f"decoder.{br}.u{u}.dense.units.{i}.conv2.weight"
    ...
    if mm:
        br = mm.group("br")
        suf = mm.group("suf")
        return f"decoder.{br}.u0.bn.{suf}"

    mm = re.match(r"^conv_out_(?P<br>np|hv)\.conv\.(?P<suf>weight|bias)$", state_key)
    if mm:
        br = mm.group("br")
        suf = mm.group("suf")
        return f"decoder.{br}.u0.conv.{suf}"

    return None


def _build_hovernet_from_legacy_desc(desc: Mapping[str, torch.Tensor]) -> torch.nn.Module:
    HoverNet = _get_hovernet_class()
    model = HoverNet()
...
def _build_hovernet_from_state_dict(state_dict: Mapping[str, Any]) -> torch.nn.Module:
    HoverNet = _get_hovernet_class()
    model = HoverNet()
...
class HoverNetInferencer:
    ...
    def _load_model(self, model_path: str) -> torch.nn.Module:
        ...
        # - Legacy format shipped by this repo's checkpoints/hovernet_*.pth
        # - Plain state_dict mappings from torch.save(model.state_dict())
        ...
        raise TypeError(
            f"Unsupported HoVer-Net checkpoint at {model_path}: got {type(obj).__name__}. "
